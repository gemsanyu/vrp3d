...
def is_overlap_any_packed_items(item_pos: np.ndarray, item_size: np.ndarray, packed_items: List[Item]):
    p_item_pos = [p_item.position[np.newaxis,:] for p_item in packed_items]
    p_item_size = [p_item.size[np.newaxis,:] for p_item in packed_items]
    if not p_item_pos:
        return False
    p_item_pos = np.concatenate(p_item_pos, axis=0)
    p_item_size = np.concatenate(p_item_size, axis=0)
