...
"""
    ...
    packing_position_dict: each vehicle has orders, each order has items, 
        each item has packing position in the vehicle's box
        now each vehicle has a dict of which item is where in the box
...
class Solution:
    def __init__(self,
        ...
        self.packing_position_dict: List[List[List[Tuple[int]]]] = []
        self.ep_list: List[List[EP]] = []
        self.weight_cost_list: List[float] = [0]*num_vehicle
        self.distance_cost_list: List[float] = [0]*num_vehicle
    ...
    #                  order_i:int, 
    #                  vec_i:int,
    #                  packing_position:int) -> Tuple[bool]:
